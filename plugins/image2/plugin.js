...
(function() {
  ...
  function widgetDef(editor) {
    ...
    function inflate() {
      var editable = editor.editable(), doc = editor.document;
  ...
  CKEDITOR.plugins.image2 = {
        ...
        link: function(shift, oldValue, newValue) {
          if (shift.changed.link) {
            ...
            if (!newValue) newEl = unwrapFromLink(link);
              ...
              var attributes = CKEDITOR.plugins.image2.getLinkAttributesGetter()(
                editor,
                newValue
              );
    ...
    checkHasNaturalRatio: function(image) {
      var $ = image.$, natural = this.getNatural(image);
  ...
  function upcastWidgetElement(editor) {
    ...
    return function(el, data) {
      var dimensions = { width: 1, height: 1 }, name = el.name, image;
    ...
    return function(el) {
      ...
      if (
        alignClasses
          ...
          : CKEDITOR.tools.parseCssText(el.attributes.style || '', true)[
              'text-align'
            ] == 'center'
      )
        return true;
  ...
  function setupResizer(widget) {
    ...
    resizer.on('mousedown', function(evt) {
      ...
      function attachToDocuments(name, callback, collection) {
        var globalDoc = CKEDITOR.document, listeners = [];
  ...
  function linkCommandIntegrator(editor) {
    ...
    CKEDITOR.on('dialogDefinition', function(evt) {
      ...
      if (dialog.name == 'link') {
        ...
        var onShow = def.onShow, onOk = def.onOk;
